...
{
    /// <summary>
    /// The supplied Pickup Afghanistan FBX contains one imported mesh node rather than four wheel
    /// transforms. This editor tool finds disconnected low/compact mesh islands, identifies the four
    /// wheel islands, splits them into independent Mesh assets and builds a physics-ready prefab.
    /// That lets WheelCollider drive visible suspension travel, steering and wheel spin instead of
    /// faking the truck with transform-only motion.
    /// </summary>
    public static class PickupWheelMeshSplitter
    {
        private const string Root = "Assets/FallenForest";
        private const string ModelPath = Root + "/Art/Models/Vehicles/Pickup/Pickup_Afghanistan.fbx";
        private const string GeneratedDir = Root + "/Generated/Pickup";
        private const string PrefabPath = Root + "/Prefabs/Pickup_Final.prefab";
        ...
        public static GameObject BuildOrThrow()
        {
            ...
            if (source == null)
                throw new InvalidOperationException($"Pickup model is missing: {ModelPath}. Import pickup-truck.zip first.");
            ...
            GameObject visualRoot = (GameObject)PrefabUtility.InstantiatePrefab(source, root.transform);
            if (visualRoot == null)
            {
            ...
            Debug.Log($"Fallen Forest: pickup split into body + 4 independent wheels and saved to {PrefabPath}.");
            return prefab;
        }
        ...
        private static List<ComponentInfo> FindConnectedComponents(Mesh mesh)
        {
            ...
            for (int i = 0; i < vertexCount; i++)
            {
                ...
                if (!groups.TryGetValue(root, out ComponentInfo component))
                {
                    component = new ComponentInfo { root = root };
                    groups.Add(root, component);
                }
            ...
            foreach (ComponentInfo component in groups.Values)
            {
                ...
                for (int i = 1; i < component.vertices.Count; i++)
                    bounds.Encapsulate(vertices[component.vertices[i]]);
        ...
        private static List<ComponentInfo> PickFourWheelComponents(List<ComponentInfo> components, Bounds whole)
        {
            ...
            // Choose the four candidates with the best score plus spatial spread in XZ. The supplied
            // model's wheel islands are four similarly sized compact components near the bottom.
            ...
            string name)
        {
            ...
            for (int sub = 0; sub < source.subMeshCount; sub++)
            {
                ...
                {
                    int a = triangles[i];
                    int root = vertexToRoot[a];
                    if (!includedRoots.Contains(root)) continue;
        ...
        private static float EstimateWheelRadiusInRootSpace(Bounds component, Transform meshTransform, Transform root)
        {
            ...
            Array.Sort(extents);
            // A tire has one narrow half-width and two similar radius dimensions. Median is robust.
        ...
        private static void AlignVehicleForward(Transform root, Transform visualRoot, List<WheelVisual> wheels)
        {
            ...
            foreach (WheelVisual wheel in wheels)
                mean += new Vector2(wheel.rootLocalCenter.x, wheel.rootLocalCenter.z);
            mean /= wheels.Count;
        ...
        {
            foreach (WheelVisual wheel in wheels)
                wheel.rootLocalCenter = root.InverseTransformPoint(wheel.visual.position);
        }
