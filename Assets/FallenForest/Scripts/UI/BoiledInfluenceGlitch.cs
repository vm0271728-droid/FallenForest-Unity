...
{
    /// <summary>
    /// Lightweight mobile-friendly persistent visual corruption after the player survives
    /// the Boiled One encounter. No post-process shader is required: short, subtle UI strips
    /// appear at irregular intervals until the current run ends or a new run is started.
    /// </summary>
    public sealed class BoiledInfluenceGlitch : MonoBehaviour
    {
        ...
        private void EndBurst(float now)
        {
            ...
            // Very rarely place a second micro-burst close to the first one. It makes the corruption
            // feel unstable without turning into a constant noisy overlay.
            if (Random.value < .14f)
                spacing = Random.Range(.22f, .7f);
        ...
        private void ArrangeStrips()
        {
            ...
            for (int i = 0; i < visible && i < strips.Length; i++)
            {
                ...
                float width = tinyBlock
                    ? Random.Range(42f, 180f)
                    : Random.Range(referenceWidth * .13f, referenceWidth * .54f);
                ...
                Color color;
                if (colorRoll < .18f)
                    color = new Color(.74f, .96f, 1f, alpha);
                else if (colorRoll < .31f)
                    color = new Color(1f, .54f, .54f, alpha * .82f);
                else
                {
            ...
            // A paired chromatic sliver beside the main strip occasionally gives a very small
            // RGB-split impression without needing a full-screen post-processing pass.
