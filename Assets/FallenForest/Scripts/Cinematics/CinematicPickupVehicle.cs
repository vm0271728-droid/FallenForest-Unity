...
{
    /// <summary>
    /// Physics-driven finale pickup. WheelCollider keeps suspension, steering and wheel spin
    /// physically coherent instead of moving the truck with transform-only animation.
    /// </summary>
    ...
    public sealed class CinematicPickupVehicle : MonoBehaviour
    {
        ...
        public bool HasStopped => !driving && stopping && Speed < .6f;
        ...
        {
            if (driving) DriveRoute();
            else if (stopping) ApplyBrakes(brakeTorque);
        ...
        {
            if (overrideRoute != null && overrideRoute.Length > 0) route = overrideRoute;
