...
namespace FallenForest.Cinematics
{
    public sealed class WakeUpSequence : MonoBehaviour
    {
        ...
        private IEnumerator Start() { yield return PlayWakeUp(true); }
        public IEnumerator PlayWakeUp(bool longVersion)
        {
            ...
            playerMotor?.SetControlsEnabled(false); cameraMotion?.SetInputEnabled(false);
            if (breathing != null && !breathing.isPlaying) breathing.Play();
            float duration = longVersion ? fullDuration : fullDuration * 0.58f, t = 0f;
            float startBlur = longVersion ? 0.78f : 0.56f;
            if (eyelids != null) eyelids.alpha = 1f; if (blurOverlay != null) blurOverlay.alpha = startBlur;
            Vector3 lowOffset = longVersion ? new Vector3(0.10f,-1.26f,0.06f) : new Vector3(0.04f,-0.58f,0.03f);
            Vector3 lowTilt = longVersion ? new Vector3(19f,0f,8f) : new Vector3(9f,0f,4f);
            cameraMotion?.SetCinematicPositionOffset(lowOffset); cameraMotion?.SetCinematicRotationOffset(lowTilt);
            while (t < duration)
            {
                t += Time.unscaledDeltaTime; float p = Mathf.Clamp01(t / duration); float eye;
            ...
            cameraMotion?.ClearCinematicTransform(); if (eyelids != null) eyelids.alpha=0f; if (blurOverlay != null) blurOverlay.alpha=0f; if (breathing != null) breathing.Stop(); cameraMotion?.SetInputEnabled(true); playerMotor?.SetControlsEnabled(true);
        }
    }
}
