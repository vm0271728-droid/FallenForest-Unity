...
namespace FallenForest.Cinematics
{
    public sealed class WakeUpSequence : MonoBehaviour
    {
        ...
        /// <summary>
        /// Used by the Boiled One encounter. The player remains locked on the creature while the
        /// camera sinks toward the ground and the eyelids close. The caller owns the forced-look target.
        /// </summary>
        public IEnumerator PlayCollapseToBlack(float duration)
        {
            ...
            Vector3 endOffset = new Vector3(0.05f, -1.18f, 0.08f);
            Vector3 startTilt = Vector3.zero;
            Vector3 endTilt = new Vector3(6f, 0f, 8f);
            float t = 0f;
            ...
            while (t < duration)
            {
                ...
                cameraMotion?.SetCinematicPositionOffset(Vector3.Lerp(startOffset, endOffset, fall));
                cameraMotion?.SetCinematicRotationOffset(Vector3.Lerp(startTilt, endTilt, fall));

                float close = Mathf.Clamp01((p - .34f) / .66f);
                close = Mathf.SmoothStep(0f, 1f, close);
        ...
        /// <summary>
        /// Opens the cinematic overlays immediately before the fullscreen screamer video.
        /// Controls stay disabled; BoiledOneSequence restores them after the wake-up.
        /// </summary>
